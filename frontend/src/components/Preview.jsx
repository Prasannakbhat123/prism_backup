...
const Preview = ({
  ...
  useEffect(() => {
    if (selectedFile) {
      ...
      const fileName2 =
        blobMapper.getFilename(selectedFile) ||
        ...
        const loadPolygonData = async () => {
          try {
            ...
            if (polygonData) {
              ...
              let loadedPolygons = JsonStorageService.convertJsonToPolygons(
                polygonData,
                selectedFile
              );
              // Normalize fileUrl to the current selectedFile (blob URL)
              loadedPolygons = loadedPolygons.map((p) => ({
                ...p,
      ...
      img.onload = () => {
        ...
        if (!parentDiv) {
          console.warn(
            "Canvas container reference is null, using default dimensions"
          );
  ...
  const joinPolygon = () => {
    ...
    // Use currentPolygon directly instead of reordering points
    setTempPolygon(currentPolygon);
  ...
  const startPan = (e) => {
    // Only start panning when using left mouse button AND either in pan tool OR holding Alt key AND zoom level > 1
    if (e.buttons === 1 && (currentTool === "pan" || e.altKey) && zoomLevel > 1) {
      setIsPanning(true);
      setPanStart({
        x: e.clientX,
        y: e.clientY,
  ...
  const endPan = () => {
    ...
    if (canvasRef.current) {
      if (currentTool === "pan") {
        canvasRef.current.style.cursor = zoomLevel > 1 ? "grab" : "not-allowed";
      } else {
  ...
  const handleMouseMove = (e) => {
    ...
    // Set cursor for pan tool but don't activate panning just by hovering, only when zoomed in
    if (currentTool === "pan" && !isPanning) {
      canvas.style.cursor = zoomLevel > 1 ? "grab" : "not-allowed";
    ...
    // Handle polygon dragging with move tool
      ...
      const currentPolygons = [...(polygons[selectedFile] || [])];
      if (
        currentPolygons[hoveredPolygonIndex] &&
        currentPolygons[hoveredPolygonIndex].points
      ) {
        const dx = x - dragStartPos.x;
        const dy = y - dragStartPos.y;

        // Update all points of the polygon
        currentPolygons[hoveredPolygonIndex].points = currentPolygons[
          hoveredPolygonIndex
        ].points.map((point) => ({
          x: point.x + dx,
          y: point.y + dy,
  ...
  const handleCanvasClick = async (e) => {
    ...
    if (isRitmMode) {
      if (!ritmReady) {
        setRitmError(
          "RITM backend is not ready. Please wait for the image to load."
        );
        return;
      }
      ...
      for (let polyIndex = 0; polyIndex < currentPolygons.length; polyIndex++) {
        ...
        try {
          ...
          if (closestPointIndex !== -1 && distance < tolerance) {
            ...
            pointFound = true;
            console.log("Point selected for dragging:", {
              polyIndex,
      ...
      if (!pointFound) {
        try {
          ...
          for (
            ...
            console.log(
              `Checking polygon ${polyIndex} with ${polygon.points.length} points for edge near (${x}, ${y})`
            );

            const edgeInfo = isPointNearEdge(x, y, polygon.points, tolerance);
            ...
            if (edgeInfo) {
              ...
              setSelectedPolygon(polygon);
              console.log(
                "FOUND! Adding point on edge of polygon:",
                polygon.name,
                "edge index:",
                edgeInfo.edgeIndex
              );
        ...
        <>
              ...
              <div className="absolute top-4 right-4 backdrop-blur-md bg-black/50 border border-gray-700 rounded-xl p-2 shadow-2xl">
                <div className="flex items-center space-x-2">
                  <button
                    ...
                    className="w-8 h-8 flex items-center justify-center bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors duration-200"
                    title="Zoom In"
                  >
                    <svg
                      className="w-4 h-4"
                  ...
                  <button
                    ...
                    className="px-3 h-8 flex items-center justify-center bg-gray-700 hover:bg-gray-600 text-white rounded-lg text-xs transition-colors duration-200"
                    title="Reset Zoom"
                  >
                    Reset
                  </button>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 min-h-0">
            <ActionButtons
              ...
              isFirstFrame={
                selectedFile ? extractFrameNumber(selectedFile) === 0 : true
              }
